          ...
          className={[
            ...
            busy
              ...
              : problemSummary.trim()
                ...
                : "bg-slate-200/75 text-slate-700 dark:bg-slate-800 dark:text-slate-300"
          ].join(" ")}
        >
          {busy ? "Host thinking" : problemSummary.trim() ? "Brief ready" : "Waiting for your answer"}
        </div>
      </div>

      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.25fr)]">
        <section className="space-y-3 rounded-2xl border border-slate-200/55 bg-white/45 p-4 dark:border-slate-700/45 dark:bg-slate-900/28">
          <div className="font-mono text-[8px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
            Host conversation
          </div>
          <div className="max-h-[20rem] space-y-2 overflow-y-auto pr-1 text-[10px] leading-relaxed text-slate-600 dark:text-slate-300">
            {problemTranscript.length > 0 ? (
              ...
              <p className="text-slate-500 dark:text-slate-400">The host has not spoken yet.</p>
            )}
          </div>
          ...
          <section className="overflow-hidden rounded-2xl bg-slate-100/55 dark:bg-white/[0.06]">
            <div className="px-4 pb-1 pt-3">
              ...
              <textarea
                ref={problemReplyRef}
                className="mt-2 min-h-[7rem] w-full resize-none border-0 bg-transparent text-[12px] text-slate-800 outline-none ring-0 placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"
        ...
        <section className="overflow-hidden rounded-2xl border border-white/20 bg-gradient-to-br from-white/[0.16] via-white/[0.08] to-slate-900/12 p-5 shadow-[0_8px_32px_rgba(15,23,42,0.07)] backdrop-blur-xl dark:border-white/10 dark:from-white/[0.07] dark:via-slate-900/35 dark:to-slate-950/65">
          ...
          <textarea
            ...
            className="mt-3 min-h-[12rem] w-full resize-none border-0 bg-white/[0.14] px-3 py-3 font-mono text-[11px] leading-relaxed text-slate-800 outline-none ring-0 placeholder:text-slate-400 dark:bg-black/25 dark:text-slate-100 dark:placeholder:text-slate-500"
            value={problemSummary}
            onChange={(e) => onProblemSummaryChange(e.target.value)}
          />
          <details className="mt-3 rounded-xl border border-slate-200/60 bg-white/35 dark:border-slate-600/50 dark:bg-slate-900/30">
            <summary className="cursor-pointer px-3 py-2 text-[9px] font-medium text-slate-600 dark:text-slate-300">
              Advanced details
            </summary>
            <div className="border-t border-slate-200/50 px-3 py-3 dark:border-slate-700/50">
